    ...
    <AppLayout title="Agriculture">
        <div class="">
            <template v-for="committee in committees">
                <template v-if="committee.name == 'Agriculture'">
                    <CommitteeHeader>
            ...
            <CommitteeMemberContainer>
                <template #header>
                    <CommitteeMemberHeader>
                        <template #description>The Committee Members, play a crucial role in the
                            agricultural development of our community. They work
                            tirelessly to address the needs and concerns of our
                            farmers, ensuring that the agricultural sector
                            operates smoothly and efficiently. Their dedication
                            and commitment to public service are commendable and
                            essential for the growth and well-being of our
                            barangay.</template>
                    </CommitteeMemberHeader>
                </template>
                <template #cards>
                    <template v-for="member in members" :key="member.id">
                        <template v-if="member.committee.name == 'Agriculture'">
                            <CommitteeMemberCard>
                                <template #image><img
            ...
            <section class="text-gray-600 body-font">
                <div class="container px-5 py-16 mx-auto flex flex-wrap flex-col">
                    <div class="flex mx-auto w-full flex-wrap mb-6">
                        <button @click="(isFirstTab = true), (isSecondTab = false)" :class="{
                'border-teal-800 text-teal-800 rounded-t bg-gray-100':
                    isFirstTab,
                        ...
                        <button @click="(isFirstTab = false), (isSecondTab = true)" :class="{
                'border-teal-800 text-teal-800 rounded-t bg-gray-100':
                    isSecondTab,
            }" class="sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none cursor-pointer tracking-wider hover:border-teal-800 hover:text-teal-800 hover:rounded-t hover:bg-gray-100">
                            Support and Funding
                    ...
                    <div v-if="isFirstTab" class="px-5">
                        <div class="flex flex-wrap -m-4">
                            <temmplate v-for="committeeDocument in committeeDocuments.data" :key="committeeDocument.id">
                                <div v-if="committeeDocument.document_type.name ==
                'Educational Materials'
                " class="p-4 md:1/3">
                                    <div
                                        class="h-full px-6 py-6 border-2 border-gray-300 rounded-lg overflow-hidden shadow-lg">
                                        <div :style="{
                backgroundImage: `url(/storage/${committeeDocument.background_image_path})`,
